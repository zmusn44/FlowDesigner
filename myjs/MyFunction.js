...
function saveObj(obj) {
	...
	$.ajax({
	    //url: projectName + "rest/ws/p/S018/handleFlowJSON/created", //20180928修改 暂时不使用接口调用
	    //url: projectName + "rule?wf_num=R_S018_B030",
	    url: projectName + CONFIG.url.save,
	    headers: {
...
function deleteNodeFromDB(idArr) {
	...
	for (i = 0; i < idArr.length; i++) {
		...
		//var url = projectName + CONFIG.url.deleteNode + "&Processid=" + processId + "&Nodeid=" + getRemovePrefixId(idArr[i]);
		/*var url = projectName + CONFIG.url.deleteNode + "/" + processId + "/" + getRemovePrefixId(idArr[i]);
		var conn = Ext.lib.Ajax.getConnectionObject().conn;
		conn.open("post", url, false);
		conn.send();*/
		$.ajax({
		    url: projectName + CONFIG.url.deleteNode + "/" + processId + "/" + getRemovePrefixId(idArr[i]),
		    headers: {
		    	sysid: CONFIG.rest.sysid,
		    	syspwd: CONFIG.rest.syspwd,
		    	userId: CONFIG.rest.userId
...
function saveAllDefaultNode() {
	...
	var s = projectName + CONFIG.url.saveAllDefaultNode + '&Processid=' + processId + '&Nodeid=';
...
function SetProperty(a, b) {
	...
	var l = projectName + CONFIG.url.setProperty + "&Processid=" + d + "&Nodeid=" + k + "&ExtNodeType=" + c + "&WF_Appid=" + top.GetUrlArg("WF_Appid");
	...
	//OpenUrl('Property.html?projectName=' + projectName + '&url=' + CONFIG.url.setProperty + '&Processid=' + d + '&Nodeid=' + k + '&ExtNodeType=' + c + '&WF_Appid=' + top.GetUrlArg("WF_Appid") + '&sysid=' + CONFIG.rest.sysid + '&syspwd=' + CONFIG.rest.syspwd + '&userId=' + CONFIG.rest.userId);
	OpenUrl(l);
}
